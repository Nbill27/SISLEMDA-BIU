...
class Arsip_m extends CI_Model
{
    ...
    public function get_pengajuan_arsip_filtered($dari = null, $sampai = null, $status = null)
    {
        $this->db->select('
        pengajuan.*, 
    ...
    ');
        ...
        $this->db->join('klasifikasi_pengajuan', 'pengajuan.id_klasifikasi = klasifikasi_pengajuan.id_klasifikasi', 'left');
        $this->db->join('disposisi', 'pengajuan.id_pengajuan = disposisi.id_pengajuan', 'left');

        if (!empty($dari)) {
            $this->db->where('disposisi.tanggal_disposisi >=', $dari);
        }

        if (!empty($sampai)) {
            $this->db->where('disposisi.tanggal_disposisi <=', $sampai);
        }

        if (!empty($status)) {
            $this->db->where('status_pengajuan', $status);
        } else {
            $this->db->where_in('status_pengajuan', ['disetujui', 'ditolak', 'tidak tersedia']);
        }

        $this->db->group_by('pengajuan.id_pengajuan'); // karena pakai MAX()
        $this->db->order_by('tanggal_status_update', 'DESC'); // urut dari yang terbaru
